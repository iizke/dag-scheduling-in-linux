...
if [ "$batch" = "-B" ]; then
	...
	echo NormalDag Scheduling >>$summary_logfile
fi
...
do_log()
{
	echo "AverageRun-(std-deviation):" > templog
...
#	echo $@
	...
	do
		...
		do
			...
			sleep 4
			mpirun -np $num_runs $time -a -f "%e %U %S %P %c %w" -o timelog schedtool -v $batch -n $j -e ./mbench-dagmpi -n $num_runs -f $script_file
