                ...
                struct libavl_allocator *allocator)
{
    ...
    if (allocator == NULL) allocator = &avl_allocator_default;
...
avl_t_first(struct avl_traverser *trav, struct avl_table *tree)
{
    ...
    x = tree->avl_root;
    if (x != NULL) while (x->avl_link[0] != NULL) {
        assert (trav->avl_height < AVL_MAX_HEIGHT);
...
avl_t_last(struct avl_traverser *trav, struct avl_table *tree)
{
    ...
    x = tree->avl_root;
    if (x != NULL) while (x->avl_link[1] != NULL) {
        assert (trav->avl_height < AVL_MAX_HEIGHT);
        trav->avl_stack[trav->avl_height++] = x;
        x = x->avl_link[1];
    }
...
avl_t_next(struct avl_traverser *trav)
{
    ...
    if (trav->avl_generation != trav->avl_table->avl_generation) trav_refresh(
                    trav);
...
static void
copy_error_recovery(struct avl_node **stack, int height, struct avl_table *new,
...
avl_destroy(struct avl_table *tree, avl_item_func *destroy)
{
    ...
    for (p = tree->avl_root; p != NULL; p = q)
        if (p->avl_link[0] == NULL) {
            q = p->avl_link[1];
            if (destroy != NULL && p->avl_data != NULL) destroy(p->avl_data,
                            tree->avl_param);
            tree->avl_alloc->libavl_free(tree->avl_alloc, p);
        }
        else {
