...
mkdir ./test_results/`uname -r` > /dev/null 2>/dev/null
echo subdir ne : $subdir
if [ "$subdir" = "" ]; then
	subdir="./test_results/`uname -r`"
else
	mkdir ./test_results/`uname -r`/$subdir> /dev/null 2>/dev/null
	subdir="./test_results/`uname -r`/$subdir"
fi

mkdir $subdir/test$thisdate

summary_logfile="$subdir/test$thisdate/summary.log"
...
read_timelog()
{
	...
	echo Loop $i-th >> $subdir/test$thisdate/timelog_backup_$j
	cat timelog >> $subdir/test$thisdate/timelog_backup_$j
	echo >> $subdir/test$thisdate/timelog_backup_$j
...
#	echo $@
	...
	do
		logfile="$subdir/test$thisdate/mbench_$j.log"
